...
export async function POST(request: NextRequest) {
  ...
  const result = await db.execute({
    sql: `INSERT INTO games (id, team_id, game_date, opponent, num_periods, players_per_period, goalie_rotation_periods, count_goalie_as_playing_time, strategy_priorities)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING *`,
    args: [
